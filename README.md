...
|---|---|---|
| **Core** | `0.1.0-beta.11` | The deterministic engine: it interprets procedures, holds runs and frame stacks, serves readings, scopes constraints, injects checkpoints, composes packages and renders the next block. |
| **Kit** | `0.1.105` | The **default turn model** built on the core: `BOOT` opens a session; `TURN` takes one operator message through `WORK` to `FINAL`; `boot.ready`, `turn.begin` and `turn.end` are its extension sockets. |
...
uv run procedural-prompting/engine/pp.py -install <workspace> -<provider>
```
...
`uv run procedural-prompting/engine/pp.py` (Windows: `\` as separator); the
...
```sh
uv run procedural-prompting/engine/pp.py -install <workspace> daneel -<provider>
```
...
|---|---|---|---|
| **[`authoring`](packages/authoring/README.md)** | `0.1.2` | The authoring face of pp: guided doors for the member, rules, offers, migrations and procedure design, plus the author reference and cheatsheet. | `kit` |
