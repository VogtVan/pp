...
Guides the design of a new or revised procedure from interaction forms first, then
front matter and body. It covers readings, context, constraints, offers, choices,
productions, feeds, checkpoints, mounts, sockets, cadence and turn frontiers through
worked examples that are exercised by the kit's bench.
...
```sh
uv run procedural-prompting/engine/pp.py -install <workspace> authoring -<provider>
```
