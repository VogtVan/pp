...
double-quoted otherwise -- what is written is what is read.
  ...
  pp-authoring.py section serve <doc> -            -> the whole `serve:` section replaced
                                                      by the rows piped one per line, each cut
...
def grammar():
    """-> the engine's reader's cut of a serve row: a blank inside a token's brackets
    belongs to the token -- a piped line is cut exactly as the reader will read it."""
    sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
    from _core import core                                  # noqa: E402 -- the bootstrap
    return core(__file__).reading.split_row
...
def dispatch(argv: list[str]) -> int:
    ...
    if verb == "get" and len(rest) == 1:
        ...
        whole = rest == ["-"]
        cut = grammar()
        rows = [list(cut(one)) for one in sys.stdin.read().splitlines()] if whole else [rest]
        if whole:
