...
class Assembling:
    ...
    def _block(self, stack: Stack, frame: Frame, instruction: Instruction,
        ...
        if not order and language.needs_options(instruction.keyword) and not instruction.options:
        ...
        block = Block(
            ...
            command=("" if order in (language.CONTINUE, language.FINAL, language.END)
                     else (order or ("INFER" if instruction.keyword == "PROVE"
            ...
            context="" if order else reading.title(frame.document.path),
