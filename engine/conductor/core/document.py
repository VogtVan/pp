...
def cycles(document: Document) -> bool | str:
    ...
    `language.FIELD` -- the DESCRIBED door, `field:` declared and no `cycle:`: a lap per
    element the agent rendered from the prose; False -- no cycle."""
...
def fields(document: Document) -> str:
    """-> the PROSE of a described door -- the `field:` key, what the door turns on, said
    to the agent in words; "" when the document declares none. The presence of the key
    declares the door, as the presence of `proc:` declares a conducted document."""
    value = document.front.get("field")
    return str(value).strip() if value is not None and str(value).strip() else ""


def nexts(document: Document) -> tuple[str, ...]:
    """-> the SUCCESSORS a document may leave to -- the names under `next:`, as a CALL
    writes them, in the order written; () when the document declares none. One name
    follows without a decision; several are decided by `choose:` or `decide:`."""
    return tuple(str(document.front.get("next") or "").split())


def chooses(document: Document) -> str:
    """-> the PROSE that decides the successor -- the `choose:` key, said to the agent
    as the brief of the injected NEXT step; "" when the document declares none."""
    value = document.front.get("choose")
    return str(value).strip() if value is not None and str(value).strip() else ""


def decides(document: Document) -> tuple[str, ...]:
    """-> the SKILL call that decides the successor -- the words of `decide:`, the
    owner's `pp-<package>` skill playing them as an `exec:` does; () when none."""
    return tuple(str(document.front.get("decide") or "").split())
...
def procedure(document: Document) -> Procedure:
    ...
    for one in instructions:
        ...
        if one.keyword == "FIELD":
            raise Refusal("field-keyword",
                          f"{document.name}: `FIELD` is engine vocabulary -- declare "
                          "`field:` in the front matter, the prose of what the cycle turns on, "
                          "and the step plays by itself when the document is entered")
        if one.keyword == "NEXT":
            raise Refusal("next-keyword",
                          f"{document.name}: `NEXT` is engine vocabulary -- declare `next:` in "
                          "the front matter, the successors, with `choose:` (the prose) or "
                          "`decide:` (a skill), and the step plays by itself when the document leaves")
