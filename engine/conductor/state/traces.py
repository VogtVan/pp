...
class Exchange:
    ...
    transitions: int = 0                     # the successors a document left to (`next:`)
    ...
    def as_dict(self) -> dict:
        return {"started": self.started.isoformat(timespec="seconds"),
                ...
                "execs": self.execs, "transitions": self.transitions,
                "switches": self.switches, "mass": self.mass,
...
def render(found: list[Run], days: int | None, key: str | None) -> str:
    ...
    tail = ["", f"{period['runs']} run(s), {period['exchanges']} exchange(s)"
            ...
            f" {period['execs']} exec(s), {period['transitions']} transition(s), {period['switches']} switch(es);"
...
def _totals(every: list[Exchange]) -> dict:
    ...
    return {
        ...
        "transitions": sum(one.transitions for one in every),
