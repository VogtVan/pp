...
overlap with the agent's own card and with the author guide `PP-DESIGN` is
...
repairs. The answer rides the command, verbatim.
*Example* —
...
documented in place (the template) and in `PP-DESIGN` key by key: the engine's
...
declared `nature`, `mount`, `payload` with the provider skill in `with`, `cycle`,
...
What counts as a miss: an answer outside `OPTIONS`, an empty production the next step
