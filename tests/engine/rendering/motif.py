...
def scenario(bench) -> int:
    ...
    if to_tool == {"PICK", "PROVE"} and "INFER" in language.FREE and "WORK" in language.FREE:
        held("the tool's channel never fuses",
             "the fusion takes FREE productions alone, and a channel reads `tool` for a "
             "PICK, a checkpoint or a CONSUMED production -- exactly what it refuses to "
             "fuse: the closing's command always answers the LAST step rendered")
    else:
